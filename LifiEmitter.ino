...
0xAA : sent a number of time to help the received compute a signal average for the thresholding of analog values0xD5 : synchronization byte to break preamble
...
void loop() {
  ...
  if(Serial.available() && transmitter_available()){ //constructing the data frame only if transmitter is ready to transmit
    ...
    if(com_buffer_nb_bytes >= lengthOfInput || c == '\n'){
